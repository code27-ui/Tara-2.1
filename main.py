...
def get_input():
    input_text = st.text_area(
        label="Provide a Detailed Description Of The Application",
        placeholder="Enter your application details...",
        height=150,
        key="app_input",
        help=" it should incluede the purpose of the application, the technologies used, and any other relevant information.",
    )
    return input_text
...
st.set_page_config(
    page_title="TARA-THREAT ASSESMENT AND RISK ANALYSIS",
    page_icon=":shield:",
    layout="wide",
...
with col1:
    ...
    sensitive_data = st.selectbox(
        label="What is the sensitivity level of the data processed by the application?",
        options=[
            "Top Secret",
            "Secret",
            "Confidential",
    ...
    pam = st.selectbox(
        label="Are Privileged Access Management (PAM) solution implemented for privileged accounts?",
        options=["Yes", "No"],
        key="pam",
    )
...
with col2:
    internet_facing = st.selectbox(
        label="Is this internet-facing application?",
        options=["Yes", "No"],
        key="internet_facing",
    )
